...
gulp.task('blazor-scripts', function () {
    try {
        ...
        var dependencies = config.blazorDependencies;
        dependencies = dependencies ? JSON.parse(JSON.stringify(dependencies).replace(/ej2-/g, '').replace(/-/g, '')) : [];
        if (!dependencies.length) {
            var packJson = JSON.parse(fs.readFileSync('./package.json', 'utf8'));
            var depKeys = Object.keys(packJson.dependencies);
            depKeys.map(dependency => {
                if (dependency.indexOf('@syncfusion') !== -1 && dependency.indexOf('ej2-base') === -1) {
                    dependencies.push(dependency.replace(/@syncfusion\/ej2/, '').replace(/-/g, ''));
                }
            });
        }
        ...
        finalJson[currentPackage] = dependencies;
